...
先在同等前部范围比较读者会带走的一个主贡献及其依据，再核对完整性。比较哪些支撑细节退出了入口、移到何处，以及删除次要信息后主张是否仍准确；不能仅因四问都能回答就判定更好读。图形按相同物理尺寸比较。续改特别检查相邻段的重复解释、摘要和方法的结论强度、计数与效果是否混淆，见[续改示例](references/actual-continuation-revision.md)。
